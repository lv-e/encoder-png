...
function editor(port) {
    const server = http_1.default.createServer((request, response) => {
        ...
        const urlComponents = url_1.default.parse(request.url, true);
        ...
        const file = Buffer.from(file64, 'base64').toString('binary').toString();
        if (urlComponents.pathname == "/stats/compressed/") {
            ...
            pn2idx_1.PNG2Indexed(file, data => {
                ...
                response.end(apiResponse({ size: data.size, png: data.png }));
            });
        }
            ...
            response.end(apiResponse({ size: sizeOriginal, png: data.toString('base64') }));
        }
    });
